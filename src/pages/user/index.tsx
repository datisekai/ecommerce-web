...
const User = (props: InferGetServerSidePropsType<typeof getServerSideProps>) => {
          ...
          <div className="flex items-center">
            <div className="mt-4 w-[70%] pb-4">
              ...
              <div className="mt-6 flex items-center">
                ...
                <TextField className="ml-4 rounded-sm border py-3 px-4 outline-none text-[#666]" control={control} error={errors} name={"name"} />
              </div>
              <div className="mt-6 flex items-center">
                <label className="w-[20%] text-right text-[#666] line-clamp-1">
                  Email
                </label>
                <TextField className="ml-4 rounded-sm border py-3 px-4 outline-none text-[#666]" control={control} error={errors} name={"email"} />
              </div>
              <div className="mt-6 flex items-center">
                <label className="w-[20%] text-right text-[#666] line-clamp-1">
                  Số điện thoại
                </label>
                <TextField className="ml-4 rounded-sm border py-3 px-4 outline-none text-[#666]" control={control} error={errors} name={"phone"} />
              </div>
              <div className="mt-6 flex items-center">
                <label className="w-[20%] text-right text-[#666] line-clamp-1">
                  Giới tính
              ...
              <div className="mt-6 flex items-center">
                ...
                <button disabled={isLoading} onClick={handleSubmit(handleUpdate)} className="mt-6 ml-4 rounded-sm bg-primary px-6 py-2 text-white transition-all hover:opacity-80">
                  Lưu
                </button>
              </div>
            </div>
            <div className="mt-4 w-[30%] border-l px-4 py-6 flex items-center justify-center flex-col">
              <LazyLoadImage
                src={preview || user?.image || generateAvatar(user?.name || user?.email || user?.phone)}
                className="mx-auto block aspect-[1/1] w-[100px] rounded-full"
              />
              <input type="file" accept="image/*" onChange={(e) => setFile(e.target.files[0])} className="hidden" name="" id="chooseImage" />
